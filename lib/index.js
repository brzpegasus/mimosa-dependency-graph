'use strict';
var assets, basePath, config, dataFile, logger, mimosaRequire, path, registration, util, _, _generateGraphData, _getDependencyInfo, _writeStaticAssets,
  __slice = [].slice;
...
_getDependencyInfo = function(mimosaConfig) {
  var dependencies, dependencyInfo, module, _ref;
  config = mimosaConfig.dependencyGraph;
  dependencyInfo = {};
  _ref = mimosaRequire.dependencyInfo();
  for (module in _ref) {
    dependencies = _ref[module];
    if (!(((config.excludeRegex != null) && module.match(config.excludeRegex)) || (config.exclude.indexOf(module) > -1))) {
